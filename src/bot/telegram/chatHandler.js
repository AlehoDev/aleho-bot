...
const commandHandler = async (bot, chatUser, text) => {
    ...
    switch (botCmd) {
        ...
        case 'resetcontext':
            geminiAI.resetChat();
            bot.sendMessage(chatUser.chatID, `🫠 Listo, ya me olvide de todo lo que hablamos...`);
            break;
    ...
    switch (text) {
        ...
        default:
            try {
                ...
                logger.info(`[TELEGRAM BOT]: USERNAME: ${chatUser.userName}  RESPUESTA: ${botResponse}`);
            } catch (error) {
...
const spamDetect = (chatUser) => {
    ...
    const diffInSeconds = Math.abs(nowTimestampDate.getTime() - maxTimestampDate.getTime()) / 100;

    console.log(`[TELEGRAM BOT]: SPAM DETECT! - ${diffInSeconds} - ${constant.SPAM_MSG_DELAY}`);
